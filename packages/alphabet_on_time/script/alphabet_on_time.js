...
$(document).ready(function() {
	if($('#alphabet_container').length > 0) {
		...
		$('#alphabet_input').keydown(function(e) {
			...
			if(key == alphabet_chars[alphabet_next])
			{
				...
				if(alphabet_next == 28)
				{
					...
					var rating = 5;
					rating = (elapsed < 10) ? 4 : rating;
					rating = (elapsed < 6.5) ? 3 : rating;
					rating = (elapsed < 4) ? 2 : rating;
					rating = (elapsed < 3) ? 1 : rating;
					$('#alphabet_rating_' + rating).show(250);					
				}			
				alphabet_next++;
				return true;
			}
		...
		$('#alphabet_reset').click(function() {
			...
			$('#alphabet_input').val('');
			$('#alphabet_input').focus();
			$('.alphabet_rating').hide(250);
		});
	}
});
